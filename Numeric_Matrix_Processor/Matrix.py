class Matrix:
    ...
    def transpose_side_diagonal(self):
        ...
        for i in range(self.rows):
            for j in range(self.cols):
                tmp_matrix[i][j] = self.matrix[self.cols -
                                               1 - j][self.rows - 1 - i]
        self.matrix = tmp_matrix
        del tmp_matrix
        return self.matrix
        ...
        if len(matrix) == 2 and len(matrix[0]) == 2:
            ...
            for fc in indices:
                ...
                for i in range(height):
                    tmp_matrix[i] = tmp_matrix[i][0:fc] + \
                        tmp_matrix[i][fc + 1:]
    ...
    def calculate_inverse_matrix(self, matrix=None):
        if matrix is None:
            matrix = self.matrix
        n = len(matrix)
        matrix_determinant = self.calculate_determinant()
        if n != len(matrix[0]) or matrix_determinant == 0:
            print("Cannot calculate inverse of this matrix.\n" +
        ...
        for r in range(n):
            ...
            for c in range(n):
                ...
                cofactor_row.append(
                    ((-1)**(r+c)) * self.calculate_determinant(minor))
        ...
        # TO DO:
        # correct returning -0.0 values in cofactor array
